...
| `!` / `!!` 面板 | 标题 `$ <command>`；关按钮 `关闭输出`，停止按钮 `停止命令`；**状态行只在非成功收尾时画**：`运行中 · 进入上下文`（muted）/ `已取消 · 进入上下文`（warning）/ `退出码 1 · 进入上下文`（error）/ `输出被截断，完整输出：<path> · 进入上下文`（warning）。**成功的命令不画状态行** —— pi 的 `setComplete` 把 exit 0 与「无 exit code」都算 `complete`，而 `complete` 不给状态行贡献任何一段（`07` D43）。`进入/不进上下文` 这半句是**我们有意的偏离**：pi 在建标题前就把 `!!` 剥掉、只用边框色 `dim`/`bashMode` 说这件事，我们依 §9 / `06 §4`「颜色不能是唯一信号」保留文字（`07` D43） | `BashPanel.BashPanel`（标题与两个按钮）、`BashPanel.statusText` / `BashPanel.statusTone`（状态行文案与颜色） |
