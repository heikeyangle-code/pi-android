...
- **级 0 `SettingsHome` 版面**：`TopAppBar("设置")` + 搜索图标 `搜索设置` → `SearchEntry`（`搜索设置` + `{n} 项`）→ `CurrentModelCard`（`当前模型` + `{model} · ◐ {level}`）→ `PiSectionHeader("设备")` + `DeviceCapabilityEntryRow` → `PiSectionHeader("扩展")` + `PiPackagesEntryRow` → `PiSectionHeader("关于")` + `PiLicensesEntryRow` → `PiSectionHeader("全部设置")` + 13 个 `GroupEntry` → 页脚 `共 {n} 项设置。搜索同时匹配标题、说明与字段名，输入 reserveTokens 或 /compact 都能直达。`
- **13 个分组（标题原文，`PiSettingsRegistry.kt:1415-1500`）**：`模型与推理`、`提示词`、`消息与网络`、`上下文与压缩`、`重试与网络`、`工具`、`会话`、`扩展与资源`、`外观`、`终端与 Shell`、`安全与信任`、`运行时与诊断`、`隐私与关于`。前 12 组是 spec §6.4 的分组；`提示词` 是本应用自己的第 13 组（`--system-prompt` / `--append-system-prompt` 两行没有 pi 的 settings 键，spec 里没有它们的落点），排在 `模型与推理` 之后。每个分组带一行动态摘要（如 `默认 {model} · ◐ {level}`、`整份替换 {n} 字 · 追加 {n} 字`、`穿插：{x} · 后续：{y}`、`主题 {theme}`、`保活：{x}`、`遥测：{x}`）。
