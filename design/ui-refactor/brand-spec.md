...
- **终端不再是禁区，但也不再投入**（2026-09-13 用户裁决，见 `03-navigation-decision.md`）：终端降格为**设置首页里的一行入口**，点进去是现有那个全屏终端页；不重做它的界面、不为它做状态设计、不让它占底部导航或输入区的位置。现有实现一律不动。
- 不许在界面上写文件路径、类名、`§` 引用等内部术语（用户既有规范）。机器内容区里 pi 的真实输出（命令、diff、工具输出）除外。
