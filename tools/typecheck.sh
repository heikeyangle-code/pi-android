...
# `font` is a real AAPT2 kind too: it comes from `res/font/**` rather than
# `res/values/**`, and it is what `FontFamily(Font(R.font.…))` resolves against.
rm -rf "$STUB"; mkdir -p "$STUB"
{
  echo "package app.pi"
  echo "// Generated by tools/typecheck.sh. Stand-in for AAPT2's R class."
  echo "object R {"
  for kind in string drawable color mipmap xml font; do
