...
- **还没定的**：`ErrorBlock` / `SystemPromptBlock` / `SkillInvocationBlock` 是否也该走 markdown —— 已逐个回 pi 源码确认：`ErrorBlock` 保持纯文本（pi 用 `new Text(...)`，`interactive-mode.ts:2829-2830`）、`SystemPromptBlock` 没有 pi 对应物、`SkillInvocationBlock` **pi 是 markdown，已改**（`components/skill-invocation-message.ts:40-45`；两处有意偏离写在块内）。`docs/gap-disposition.md` §10.1 是这三条的账本。
