...
| # | 欠账 | 严重度 | 一句话 |
|---|---|---|---|
| **D1** | **会话内分支切换 + 分支摘要**（pi 的 `navigateTree`） | **高** | pi 有 `session.navigateTree()` 与 `/tree`；RPC 线上**没有任何命令**能触发它。App 的树屏只能 `fork` 出**新会话文件**，并且已经渲染 `BranchSummaryBlock`，但那条分支摘要**永远不会有输入**。 |
...
### D2 — 扩展注册的 CLI flag 透传（`registerFlag` / `getFlag`）｜严重度：低
...
3. **`fork` 的 `position:"before"|"at"`**：RPC 侧 `fork` 不传 position（用默认），`clone` 传 `"at"`（`modes/rpc/rpc-mode.ts:613-630`）。我没有确认默认值是 `before` 还是 `at`（要读 `runtimeHost.fork` 的实现），因此"D1 之外 fork 语义是否也有一处差异"**未确认**。`{未确认}`
