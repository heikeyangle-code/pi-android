...
| F25 | DEAD CODE | the `else -> NoticeBlock("暂不支持的内容块")` branch is unreachable | `ui/blocks/BlockRenderer.kt` → 行为 | — | **applied both halves (uncommitted)** — **app:** `ui/blocks/BlockRenderer.kt:83-88` — the `else` and its `NoticeBlock("暂不支持的内容块…")` are deleted, and the KDoc records why the branch could never run (`TranscriptItem` is sealed with exactly the 14 kinds matched above, so an unknown *item* cannot exist and a future subtype must fail the build instead of silently rendering as a notice). **rpc (point authorisation, extended this pass to the matching delta branch):** `rpc/Transcript.kt:961-984` — `PiEvent.Unknown`'s non-entry path no longer returns `TranscriptChange.None`; it appends the same kind of deduplicated `Notice` the file already uses for an unknown assistant delta (`:1147-1157`), and both now carry **one shared user-facing sentence** — `收到一条当前版本不认识的消息；升级 App 后可能可见。` — instead of the wire names (`未知的事件：<type>` / `未知的流式事件：<kind>`), per the rule that the UI must not explain internal implementation. Consistency was the reason for the fix: an unknown delta was visible and an unknown event was silent in the same reducer. **Consequence, deliberate:** because the two sites share the sentence, their dedupe checks now agree, so at most one such row appears per transcript however many unknown kinds arrive (previously one per kind) | — |
...
against the code: **implemented/applied 20 · blocked 9 · partial 4 (rpc half done, UI half not) ·
closed 1**. The IDs, so the numbers can be checked rather than believed:

```
implemented/applied (20)  F1 F2 F3 F4 F5 F6 F8 F11 F12 F15 F17 F19 F23 F24 F25 F26 F30 F31 F32 F33
blocked             ( 9)  F7 F9 F13 F14 F20 F21 F22 F28 F29
partial             ( 4)  F10 F16 F18 F34
closed              ( 1)  F27
```
...
| `rendering-review.md` rows (§10) | 20 | 4 | 0 | 9 | 0 | 1 | 34 |
