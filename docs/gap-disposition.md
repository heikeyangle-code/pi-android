...
| **A1** `SkillInvocationBlock` | expanded = `PiMarkdownText` over the skill body; collapsed stays header-only, which is pi's collapsed state too (pi's collapsed branch prints one `[skill] name (… to expand)` line and no body) | pi `components/skill-invocation-message.ts:40-45` (expanded `Markdown`) and `:47-52` (collapsed line); app `ui/blocks/SkillInvocationBlock.kt:65-89` | **done** — two recorded deviations: pi's `**name**` header (`:41-42`) is not repeated because this card's header carries `/skill:name` in both states, and pi's `customMessageText` colour (`:44`) equals `text` in both built-in themes (`theme/dark.json:43`, `light.json:42`) |
...
| RR F32/P9 (markdown config rebuilt per frame) | **Implemented — the wrapper is gone, the builders are pure** | `remember { piMarkdownColors() }` was illegal because the builders are `@Composable` and `remember`'s calculation lambda is `@DisallowComposableCalls`. The fix is the one that entry asked for: `PiMarkdownText` reads the four inputs and calls pure constructors (`PiMarkdown.kt:76-88`), `PiMarkdownTheme.kt` builds `DefaultMarkdownColors` / `DefaultMarkdownTypography` / `alert` colours and holds `MarkdownPadding`/`MarkdownDimens` implementations as constants (`:123`, `:161`, `:189`, `:225-265`, `:267-299`), and `piMarkdownComponents()` is simply not `@Composable` any more (`PiMarkdownComponents.kt:98-103`) so it is remembered directly. No composable call sits inside any `remember`. Not verifiable here — CI is the only build. |
