...
| `applied (uncommitted)` | The work is **in the working tree but not on CI** and not necessarily wired end to end. Used by the package-management rows (#51, #54) for exactly that reason: "done" is a claim only a green build supports. |
...
Summary: **fixed 1 · patch-ready 23 · blocked 11 · in-flight 13 · applied (uncommitted) 2 · limit 14 · closed 5** (69 rows).
The two `applied (uncommitted)` rows are #51 and #54 (`packages/**`, owner *包管理代理*): the screen is
mounted, the built-in/user distinction is drawn and labelled as the app's own claim, and the install
command now binds the same agent dir as the engine (`docs/known-gaps.md` §K5). They are not `fixed`
until CI has built them, and the git-source half of #51 stays blocked on §K2.
