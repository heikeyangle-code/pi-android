...
| F10 | GAP | no live token/context indicator; all three `usage` payloads dropped | `ui/**` + `rpc/**` → 行为 + RPC | — | **rpc half implemented, UI half blocked (new UI)** — all three payloads now reach the reducer: `Transcript.kt:695` (`lastUsage`) written from `message_update` (`:953`) and `message_end` (`:771`), plus `compaction_end.result.usage` / the persisted entry's `usage` onto `CompactionMarker` (`:133`, `:1377`, `:1693`); parsed at `Events.kt:415`, `:426` and `Responses.kt:370`. Test: `FidelityFixesTest:719`. But a grep over `app/src/main` finds **no reader** of `TokenUsage`/`lastUsage` (only the comment at `PiEngineSession.kt:265`), so nothing is painted: spec §4.1's 32 dp status row and §4.9's 70 %/90 % states are still a `ui/**` change. | — |
...
| F16 | GAP | images returned by a tool are flattened to the literal text `[image]` | `rpc/Transcript.kt` + `ui/blocks/` → RPC + 行为 | — | **rpc half implemented, UI half not wired** — the bytes survive the parser: `Events.kt:454` + `imageBlocks` (`:639`) feed `ToolCall.images` (`Transcript.kt:82`), from the live `tool_execution_end` (`:1144`) and from replay's toolResult entry (`:1555`); `contentText` still writes `[image]` but only on the plain-text path. Test: `FidelityFixesTest:688` (image kept alongside `outputTruncated`). **But `ToolCall.images` has no consumer in `app/src/main`** (`ToolCallBlock.kt` never reads `images`; `ImageGridBlock` is used only by `UserMessageBlock.kt:48`), so a tool's picture still is not drawn. | #62, A3 |
...
| F18 | GAP | compaction/branch-summary cost is parsed then dropped | `rpc/Transcript.kt` + `ui/blocks/` → RPC + 行为 | — | **rpc half only half done — verified, not finished** — the compaction cost is carried (`CompactionMarker.usage`, `Transcript.kt:133`; live `compaction_end.result.usage` `:1377`, persisted entry `:1693`) with a test (`FidelityFixesTest:740`). Missing: (1) **branch summaries carry no usage at all** — `BranchSummary` (`:139-144`) has no field and `onBranchEntry` ignores `entry.usage`, while pi emits the same notice for both kinds (`interactive-mode.ts:3791-3792` builds the `compaction_cost` item for `compaction` *and* `branch_summary`, `:3708-3711` dispatches it, `:3430-3436` is the live path); (2) the visible notice row is not produced anywhere. **The row must not be appended by the reducer**: pi gates it on `settingsManager.getShowCacheMissNotices()` (`interactive-mode.ts:3804`, default **false** at `core/settings-manager.ts:120`), and the reducer has no access to settings — the app owns the gate. The app already has the exact row (`PiSettingsRegistry.kt:331`) but **no reader**, so it is a dead setting (I5-class). Wording/format if the app ever draws it: `interactive-mode.ts:3803-3814` (`${label}: ${formatTokens(tokens)} tokens billed (~$0.03)`, label `Compaction` / `Branch summary`). | A1 |
...
| F22 | DEAD CODE | the `system-prompt` block kind cannot be produced | `rpc/Transcript.kt` → RPC | — | **rpc-side fix is impossible alone; labelled app-only instead** — pi has no `system_prompt` entry (`session-manager.ts:145-155`) and **no RPC command returns the prompt** (`rpc-types.ts:20-71`; `get_state`'s `RpcSessionState`, `:96-109`, has no prompt field), while pi never renders the prompt text at all (its `/context` listing prints only the source path, `interactive-mode.ts:1715-1722`, and the only reader of the text is the extension hook at `:2068`). `onSystemPrompt` has no caller in `app/**` (tests only). Both the item (`Transcript.kt:184`) and the hook (`:1883`) now say exactly that. Deleting is a **coordinated** change — item + `ui/blocks/SystemPromptBlock.kt` + `BlockRenderer.kt:71` + spec §7.4's row + the test — because deleting the item from `rpc/` alone makes `ui/**` fail to compile. | — |
| F23 | DEAD CODE | `outputTruncated` is never assigned, but is read for a label | `rpc/Transcript.kt` + `ui/blocks/ToolCallBlock.kt` → RPC + 行为 | — | **implemented** — `detailsTruncated` (`Transcript.kt:506`) reads `details.truncated` *and* `details.truncation`/`details.truncation.truncated`, and both finalize paths assign it (`:1176` update, `:1193` append), so `ToolCallBlock`'s 已截断 label is reachable. pi really does put it there: `core/tools/bash.ts:268` (`truncation: snapshot.truncation.truncated ? snapshot.truncation : undefined`), `core/tools/read.ts:156-165` (`details = { truncation }`), shape at `core/tools/truncate.ts:18-30`. Tests: `FidelityFixesTest:688` (truncated ⇒ true) and `:706` (`"truncation":null` ⇒ false). | — |
| F24 | DEAD CODE | 8 `message_update` delta kinds parse then fall into `None` | `rpc/Events.kt`, `rpc/Transcript.kt` → RPC | — | **partly implemented — 1 fixable kind still open, 4 correctly dropped (verified)** — the review's prescribed fix is in: the authoritative payloads now overwrite the row (`text_end.content`, `Transcript.kt:1002`; `toolcall_end.toolCall`, `:1037`), tests `FidelityFixesTest:660`, `:674`. **Still open and symmetric to `text_end`:** `thinking_end` carries `content` on the wire (`ai/src/types.ts:553`; `modes/json-event.ts:43-71` strips only `partial`) and the app drops it at `Transcript.kt:1076` — a lost thinking delta can therefore leave the row wrong. The other four are right to stay dropped: `text_start`/`thinking_start` keep only `contentIndex` and `start` only its type (their content lives in the cumulative `partial`, stripped by `json-event.ts:43-46`), `done.reason` is already covered by `message_end.stopReason` (`Events.kt:425`), and `toolcall_delta` would require porting pi's tolerant streaming-JSON parser (`ai/src/utils/json-parse.ts:104-124` + the `partial-json` dependency) for a row pi's own renderer does not build from the delta at all (`interactive-mode.ts:3244-3277` renders the accumulated message). | — |
