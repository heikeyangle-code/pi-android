...
| 13 | `--api-key` | CLI-ONLY | **ASSIGNED** | **E9** + in-flight `packages/PiConfigFiles.kt` / `PiCredentialService.kt` | **2026-09-14：** 本行原先以「`ui/chat/PiSlashCommands.kt:165` `/login` is `TerminalOnly`」起头——那个面板行与 `TerminalOnly` 这个 action 都已删除，该引用失效。**stale citation fixed — the writers are now reached**: `packages/PiConfigFiles.kt:308` `fun setApiKey(providerId: String, key: String): String?` is called from `PiCredentialService.kt`, which `ui/settings/PiCredentialScreen.kt:125` constructs, and the row that opens that screen is `PiSettingsRegistry.kt:367` via `PiSettingsStack.kt:146`. Same residual as #49: uncommitted, no device pass | `cli/args.ts:108`; `docs/providers.md:62-107` | applied (uncommitted) |
...
| 26 | scoped models selector (`/scoped-models`) | **REVERTED 2026-09-14** — was **IMPLEMENTED** (applied, uncommitted — not on CI) | — | — | **2026-09-14：用户裁决「点一下需要跳转到设置导航的全删掉」，于是本行的实现（面板行 + `PiCommandAction.OpenModelScope` + `NavRequest.SettingsFocus` 的唯一生产者）全部删除。** 下面那段 Applied 记录保留为历史；现状是：面板里没有 `/scoped-models`，能力只剩 `enabledModels` 设置行（`PiSettingsRegistry.kt:339`），pi 的 `showModelsSelector()` 覆盖层从未被复刻。**Applied（历史）:** the command was `PiCommandAction.OpenModelScope` (`ui/chat/PiSlashCommands.kt:59`, set at `:146`), which both entry points dispatch to `NavRequest.SettingsFocus("enabledModels")` (`ui/screens/ChatScreen.kt:346-349`; typed text routes there too via `ui/chat/SlashPalette.kt:232`), and the settings stack opens that group with the row highlighted (`ui/settings/PiSettingsStack.kt:79-84`, `ui/PiRoot.kt:203-204`). pi's target is `showModelsSelector()` (`interactive-mode.ts:2975-2978`, method `:5024`), which toggles `settings.enabledModels` (`settings-manager.ts:1316-1326`). | `keybindings.md` §Scoped Models Selector; `interactive-mode.ts:2975-2978` | reverted 2026-09-14 — palette row + `PiCommandAction.OpenModelScope` deleted |
...
| 29 | `/import` | MISSING-TERMINAL-ONLY | **LIMIT** | **F2** / §2.9 | **2026-09-14：本行原先引 `ui/chat/PiSlashCommands.kt:127` `PiCommandAction.TerminalOnly` —— 那个 action 已整个删除，引用失效**（档位不变：该档说的是 pi 没有 `import` 这个 `RpcCommand`）。app 侧能力并不缺：`/import` 面板行是保留的 11 条之一（`PiCommandAction.ImportSession`），走文件选择器 → `PiSessionViewModel.importSession` → 把选中的 JSONL 拷进会话目录后 `switch_session`（`rpc-types.ts:61`）。是否据此改档/改 disposition 留给下一轮裁决 | `interactive-mode.ts:2998,6107`；pi 无 `RpcCommand` | limit |
| 30 | `/share` | MISSING-TERMINAL-ONLY | **LIMIT** | §2.9 | **2026-09-14：面板行已删除**（A 组，用户裁决）。本行原先引 `ui/chat/PiSlashCommands.kt:129` 的 `TerminalOnly` 行，随该 action 一起失效；能力仍未交付（要 `gh` CLI，Android 无） | `core/slash-commands.ts:27`; no `RpcCommand` | limit |
| 31 | `/login`, `/logout` | MISSING-TERMINAL-ONLY | **LIMIT** | **F3** / §2.9 | **2026-09-14：面板行已删除**（C 组，用户裁决）。本行原先引 `ui/chat/PiSlashCommands.kt:138-139` 两处 `TerminalOnly`，均已不存在；OAuth 仍只在 pi 的交互式 shell 里 | `core/slash-commands.ts:33-34`; no `RpcCommand` | limit |
| 32 | `/reload` | MISSING-TERMINAL-ONLY | **LIMIT** | **B6** | **2026-09-14：面板行与 AppBar 刷新图标都已删除。** 本行原先引 `ui/screens/ChatScreen.kt:240-242` 的 `notifyTerminalOnly(reloadCommand())` —— 那个函数、那个图标和 `reloadCommand()` 都已不存在（图标只会回一句「终端里才有」，是个假动作）。能力仍未交付：RPC 无 `reload`，替代是「设置 → 运行时与诊断 → 进程 → 重启引擎」 | `rpc-types.ts:20-74`; built-ins are excluded from `get_commands` | limit |
| 33 | `/changelog`, `/hotkeys`, `/quit` | MISSING-TERMINAL-ONLY | **LIMIT** | — | **2026-09-14：三条面板行已删除**（A 组，用户裁决）。本行原先引 `ui/chat/PiSlashCommands.kt:133-134,144` 的 `TerminalOnly`，随该 action 一起失效；三条能力在本平台都不成立（TUI 文本面板 / 触屏无键位 / 无语义） | `core/slash-commands.ts:31-32,42`; no `RpcCommand` | limit |
...
| Whether the 12 `MISSING-TERMINAL-ONLY` rows are actually usable in this app's terminal tab | **2026-09-14：** 原先写作「the 12 `TerminalOnly` rows」——那是已删除的 action 名；这里问的从来是**评级**为 MISSING-TERMINAL-ONLY 的那些行。The tab exists (`PtyLauncher.Kind.PiTui`, reachable at `ChatScreen.kt:694`) but `known-gaps` C2 records the hand-written VT emulator as never run on a device. | The device pass already listed as C1/C2. |
...
### P10 — #26 `/scoped-models` is terminal-only — **APPLIED (uncommitted, not on CI)**, then **superseded 2026-09-14**

> **2026-09-14 — this patch was undone, not broken.** 用户裁决「点一下需要跳转到设置导航的全都删掉」，
> 于是 `/scoped-models` 面板行、`PiCommandAction.OpenModelScope`、以及 `NavRequest.SettingsFocus`
> 的唯一生产者全部删除（见 `PiSlashCommands.kt` 的 A/B/C 分组注释）。P10 当时修的是「面板说
> 『仅终端』却说错了」这件事——该行现在不存在了，所以这一条不再是一个缺陷；`enabledModels`
> 设置行（`PiSettingsRegistry.kt:339`）仍在，选择器组件本身从未实现。**下面这一段保留为当时的记录。**
...
- `ui/chat/PiSlashCommands.kt:146` — the `/scoped-models` row now carries it, so the palette no
  longer labels the command 「仅终端」 (at the time `SlashPalette.kt` marked `TerminalOnly` rows only;
  both the action and that marking are gone as of 2026-09-14);
...
| 26 | patch-ready P10 | **reverted 2026-09-14** — the patch (`PiCommandAction.OpenModelScope` → `NavRequest.SettingsFocus(enabledModels)`) was undone by the user's ruling that no palette row may lead into Settings |
...
| F6 | GAP | extension `custom` entries render nothing | `rpc/Transcript.kt` → RPC | RR-P4 | implemented — RR-P4 applied — (`onCustomEntry` for `"custom"` entries) — **then removed 2026-09-24**: the user ruled the row out (it was the App's own rendering of an extension payload, printed as 「扩展状态：<customType>」 plus a key/value table under every tool card). `onEntry`'s `custom` arm now returns `TranscriptChange.None`, matching pi's own silence when no renderer is registered (`interactive-mode.ts:3558-3561`); the entry is still read and kept | — |
...
| #26 / P10 (`/scoped-models`) | **Done, then reverted 2026-09-14** | It *was* reachable: `PiCommandAction.OpenModelScope` (`ui/chat/PiSlashCommands.kt`) on the `scoped-models` command, carried as a settings-highlight nav request through `PiRoot` (`NavRequest.SettingsFocus(enabledModels)`) into `PiSettingsStack`. **2026-09-14：** 用户裁决「命令面板里点一下要跳转到设置导航的全删掉」，该面板行与 `PiCommandAction.OpenModelScope`、以及 `NavRequest.SettingsFocus` 的唯一生产者一并删除；`enabledModels` 设置行仍在（`PiSettingsRegistry.kt:339`）。 |
