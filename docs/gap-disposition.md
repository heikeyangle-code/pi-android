...
| 10 | `pi config` (per-resource enable/disable) | CLI-ONLY | **UNRECORDED** | — | `ui/settings/PiSettingsRegistry.kt:807` `key = "packages[].autoload",` — whole-package only; no per-resource switch anywhere | `package-manager-cli.ts:278-289` | blocked |
...
| # | Capability | Grade | Disposition | Owner / entry | App file proving the deficiency still exists | pi citation | Status |
|---|---|---|---|---|---|---|---|
| 51 | npm/git packages (`packages`) management UI | PARTIAL | **ASSIGNED** | **B5** + **E7**, owner *包管理代理* (§G) | Was: no caller. **Now mounted and effective** — `packages/PiPackagesHost.kt:129` is the screen's caller (settings entry row) and, since §K5, the command it runs binds the *same* agent dir as the engine (`GuestCommand.kt:198-206`), so a successful `pi install` is actually read by the running engine. **Still not git-capable:** the runtime has no git (§K2), and `SPEC_HINT` no longer advertises it (`PackageStrings.kt:41`). **Second entry point added (this pass, uncommitted):** the settings `packages` row (`PiSettingsRegistry.kt:792`) is now `readOnly = true` and jumps here (`PiSettingsStack.kt:148-153`) instead of opening a list editor — hand-editing that array bypasses `pi install`'s resolve/download step and can leave a spec that looks installed but was never fetched | `docs/packages.md:22-39`; not in the `RpcCommand` union | applied (uncommitted) |
| 52 | package resource enable/disable (`pi config`) | CLI-ONLY | **UNRECORDED** | — | `PiSettingsRegistry.kt:807` `key = "packages[].autoload",` — whole-package only | `package-manager-cli.ts:278-289` | blocked |
...
| 54 | built-in vs user-installed package distinction | MISSING-GUI | **ASSIGNED** | **E7**, owner *包管理代理* (§G) | Done: `packages/PiPackageModel.kt:63-119` (`PiBuiltinExtension`, transcribed from `assets/pi-extensions/`) + `packages/PiPackagesHost.kt:457-470` (three-state presence in both agent dirs) + `packages/PiPackagesScreen.kt:235-328` (built-in block, **no remove button**). **pi itself has no such distinction** and the UI says so: auto-discovery labels everything `source:"auto"`/`scope:"user"` (`package-manager.ts:2352-2362`, `:2470-2475`) and `pi list` reads only settings' `packages` (`package-manager-cli.ts:970-1002`), so the label is declared as the app's own | — (app-side) | applied (uncommitted) |
...
| 56 | all `Action`-kind rows are inert (19, not 20 — `app.device.sessionOverride` was deleted with the other `app.device.*` rows in `9a16271`) | MISSING-GUI | RECORDED-NO-OWNER (I) | **I2** | **applied (uncommitted), no CI yet.** `PiRoot.kt:188` passes `onRunAction` and every remaining row now has a decision, none left on the "还没有接入实现" fallback: **4 implemented** (`app.credentials.apiKey` + `app.localModels.manage` → `PiCredentialScreen` via `PiSettingsStack.kt:146-147`; `app.compaction.runNow` → RPC `compact` `PiRoot.kt:205`; `app.security.emergencyStop` → `stop()`+`abortBash()` `PiRoot.kt:216-219`), **3 navigate to the original TUI** for behaviours pi implements only there (`app.credentials.oauth` / `app.sessions.import` / `app.about.changelog`, `PiRoot.kt:62-79` + `:189-201`), **12 removed** with the reason left at the deletion site (`PiSettingsRegistry.kt:729`, `:1518`, `:1553`, `:1586`, `:1594`, `:1686`). The fallback branch survives only as a regression guard and is unreachable for every registered row (`PiRoot.kt:227-231`). Per-row evidence: `known-gaps.md` **§I2** | — (app-side) | applied (uncommitted) |
...
| 58 | update checks | MISSING-GUI | RECORDED-NO-OWNER (I) | **I2** | **applied (uncommitted) — the row was removed, not wired.** pi's own check is switched off by this app on purpose (`PiEngineHost.kt:305-306` sets `PI_SKIP_VERSION_CHECK`; the gate is `utils/version-check.ts:97`), and its replacement is an npm self-update (`package-manager-cli.ts:1033-1068`) while this app's engine is an APK asset re-extracted per runtime revision (`RuntimeProvisioner.kt:505-528`, `:122`) — so an in-guest update is overwritten or diverges. Nothing is left to check; the read-only 「pi 版本」 row (`PiSettingsRegistry.kt:1508`) still reports the version. pi's check is not reachable over RPC either | `docs/environment-variables.md`; `cli/args.ts` | applied (uncommitted) |
...
| 67 | runtime/ops diagnostics (`app.runtime.*`) | MISSING-GUI | RECORDED-NO-OWNER (I) | **I2** | **applied (uncommitted) — all six `Action` rows removed, not wired.** `checkUpdate`, `rollback`, `cleanNpmCache`, `phantomKillerGuide`, `logViewer`, `exportDiagnostics` are gone (`PiSettingsRegistry.kt:1518`/`:1553`/`:1586`/`:1594`). pi has no counterpart for any of them (no rollback/cache-clear/diagnostics subcommand), and the app has no level-filtered log sink or update channel to back them. **The old "8 `app.runtime.*` Action rows" count was already wrong** — only these six were `Action`; `piVersion`/`nodeVersion`/`rootfsUsage` are read-only `Text`, `keepAlive`/`safeMode` are `Switch`, `wakeLock` is `Value` | — (app-side) | applied (uncommitted) |
...
| 10 | `pi config` per-resource enable/disable has no app path (`PiSettingsRegistry.kt:807`) | A user cannot switch off one skill/prompt/theme/extensions entry inside an installed package from the GUI — only the whole package's `autoload`; per-resource filtering means hand-editing `settings.json`. |
...
| 52 | `pi config` again, recorded as a skills/packages row (`PiSettingsRegistry.kt:807`) | Same consequence as #10: per-resource enable/disable is unreachable from the GUI. |
...
|---|---|---|---|---|---|---|
| F1 | DEFECT | a message sent mid-turn never reaches the screen | `ui/PiSessionViewModel.kt` → 行为 | RR-P1 | **implemented (uncommitted)** — already in the tree from the behaviour agent's pass (§11), verified by reading both sites this pass: `PiSessionViewModel.kt:1545` (`send`, mid-turn branch) and `:1595` (`sendFollowUp`) call `engine.echoUserPrompt(...)` and then `syncTranscript(engine, engine.publication.value)`, so the queued row is visible without waiting for the collector. **Not P1 as written:** the diff's `engine.transcript.onUserPrompt(...)` mutates the reducer without publishing, so no consumer would see the row; `echoUserPrompt` is the publishing form (`PiEngineSession.kt:450`) and is what `prompt` itself calls (`:434`). No code change was needed | #2/#3/#17 (my P4) |
...
| F5 | DEFECT | every `entry_appended` is projected twice | `ui/PiSessionViewModel.kt` → 行为 | RR-P2 | **implemented (uncommitted)** — already in the tree from the behaviour agent's pass (§11), verified by reading it this pass: `PiSessionViewModel.kt:941` is `is PiEvent.EntryAppended -> Unit`, i.e. P2's deletion of the `onEntry` + `syncTranscript` pair, and the comment there records why (the engine's `onEvent` already projects and publishes for every non-`response` event, so a second projection would double-render the moment the reducer gains a `custom` case — F6). No code change was needed | — |
| F6 | GAP | extension `custom` entries render nothing | `rpc/Transcript.kt` → RPC | RR-P4 | **implemented (f9b8caa)** — `onEntry`'s `"custom"` case (`rpc/Transcript.kt:1518`) → `onCustomEntry` (`:1851`) appends one muted `Notice`: `customType` plus a compact `data` capped at 200 chars (`:506`); `display:false` suppresses it (pi's `CustomEntry` has no such field, `session-manager.ts:104-108`). pi draws it on both paths — live `entry_appended` (`interactive-mode.ts:3202-3207` → `:3557-3562`) and replay (`:3703-3707`, entry list at `:3786-3789`) — through a renderer that is a live TUI component and cannot cross RPC, so type + payload is the honest fallback. Tests: 6 in `TranscriptBlocksTest`. **Not final shape:** a dedicated card needs rpc-side `CustomEntry` item + `ui/blocks/CustomEntryBlock.kt` + a `BlockRenderer` branch **in one change, keeping the `else`** (F25's fix deletes that `else`, which would then not compile against a new sealed subtype). | — |
...
| F8 | PERF | `tool_execution_update` applied unthrottled (spec asks 200 ms) | `rpc/Transcript.kt` → RPC | RR-P3 | **implemented (f9b8caa rpc + 22c6f49 engine)** — the reducer throttles per `toolCallId` to 200 ms (`rpc/Transcript.kt:630`, `:687`, `:1164-1192`); the stored row is always current, only the publication is coalesced. `finalizeTool` (`:1258`) and `failTurn` (`:1393`) publish unconditionally and `finishStreaming` (`:1979`, `:2002`) flushes the held row at every turn boundary, so the last chunk of a burst cannot be lost. The engine skips the publish for exactly this event (`PiEngineSession.kt:292-293`), which is what makes the win real. Tests: 6 in `TranscriptReducerTest` (incl. the turn-boundary flush). **Remaining half:** `PiSessionViewModel.syncTranscript` still rebuilds the whole `UiState` per revision (`:745-778`); consuming `publication.changedIndices` is the UI owner's part (the spec's 200 ms already holds without it, because the engine no longer publishes at all). | — |
| F9 | PERF | session replay and entry projection run on the main thread | `ui/PiSessionViewModel.kt` → 行为 | — | **blocked (ruled this pass, deliberately not done)** — needs **rpc-side locking or a pure `project()`** so the projection can leave the frame thread; splitting only `PiResponses.entries()` in the UI is the cheap half and is a net negative, **so it is deliberately not done**. Reasoning, verified by reading the tree: `PiEvent.Response.data` is already a `JsonElement` (`rpc/Events.kt:104`), so `PiResponses.entries` (`rpc/Responses.kt:477-480`) is a cast + `mapNotNull`; the expensive half is `engine.seedHistory(...)` → `seedFromHistory`'s projection (`rpc/Transcript.kt`), which the review itself requires on Main because it mutates the reducer the publication collector reads. (The prepared `withContext(Dispatchers.Default) { PiResponses.entries(response) }` patch for `replayHistory` lives in this pass's report only; it was not applied, and the file is outside this agent's writable set in any case.) | — |
...
| F11 | DEFECT | block margins and rhythm are ~2× the spec | `ui/blocks/BlockChrome.kt`, `ui/screens/ChatScreen.kt` → 行为 | RR-P6 | **implemented (uncommitted)** — `ui/blocks/BlockChrome.kt:44-65`: `BlockColumn` no longer pads itself (the `.padding(horizontal = PiSpacing.screen, vertical = PiSpacing.unit / 2)` is gone, KDoc now says the list owns the margin), and `ui/screens/ChatScreen.kt:520-539`: the default `blockSpacing` is the spec's 16 dp (`PiSpacing.screen`, was `PiSpacing.unit` = 18) with `compact`/`cozy` still scaling around it, `contentPadding` vertical likewise 16. Net: side margin 16 dp instead of 32 (spec §7.4 `assistant-text` 左右内边距 0) and the inter-block gap the density setting's single value instead of 18 + 9 + 9 = 36. `DateSeparatorBlock`/`ModelChangeBlock` do not use `BlockColumn` and keep their own padding, unchanged as the review notes | — |
| F12 | DEFECT | dark-theme `dim` text is 2.1–2.9:1, under the spec's 3:1 floor | `ui/blocks/UserMessageBlock.kt`, `BranchSummaryBlock.kt` → 行为 | RR-P7 | **implemented (uncommitted)** — RR-P7, the two sites the review computes as under spec §9's 3:1 metadata floor: `ui/blocks/UserMessageBlock.kt:54-65` (the bubble's own timestamp, 2.11:1) now uses `palette.userMessageText.copy(alpha = 0.62f)` instead of `palette.dim`, and `ui/blocks/BranchSummaryBlock.kt:64-73` (the branch id, 2.89:1 on `customMessageBg`) now uses `palette.muted` (4.20:1 on a card surface). `PiPalette` is untouched — the values are pi's own tokens and F13/F14 own any palette decision. The third `dim` site the review lists (`ModelChangeBlock`/`NoticeBlock`/`DateSeparatorBlock` on `pageBg`, 3.08:1) already clears the floor and is left alone | — |
...
| F15 | GAP | user messages render raw Markdown source | `ui/blocks/UserMessageBlock.kt` → 行为 | — | **implemented (uncommitted)** — the body now goes through `PiMarkdownText(markdown = item.text, textColor = palette.userMessageText)` (`ui/blocks/UserMessageBlock.kt:44-67`), which is pi's composition exactly: `components/user-message.ts:40-56` calls `Markdown` with `defaultTextStyle.color = userMessageText`, and that is what `PiMarkdownText`'s `textColor` parameter feeds (`ui/render/PiMarkdown.kt:69-73`, :92-97). The now-unused `MaterialTheme` import was removed; the timestamp `Text` (and its import) stays. **Recorded limit:** pi also passes `preserveOrderedListMarkers`/`preserveBackslashEscapes` at that call site, and neither is exposed by the renderer wrapper, so ordered-list markers are the library's to decide — the KDoc says so instead of promising parity | A1 (same renderer) |
...
| F19 | GAP | five tap affordances unreachable — callbacks never supplied | `ui/screens/ChatScreen.kt` → 行为 | RR-P10 | **implemented (uncommitted)** — both branches of the review's minimum were taken, per affordance. **Wired (target exists):** `ChatScreen.kt:553-601` passes `onModelClick` (RR-P10: `refreshModels()` + `ChatSheet.Model`) and `onBranchClick` (`refreshTree()` + `requestNav(NavRequest.SessionTree)`, the same target the palette's 会话树 action uses). Because the branch row's tap now jumps, `BranchSummaryBlock.kt:77-87` moved the expand onto the `ExpandLabel` (nested `clickable`, so the summary body stays reachable). **Deleted (no target in the app — the review's other allowed branch):** `onDiffOpenFull` + the gated 「全屏」 label (`DiffBlock.kt:35-40`, `:80-91`), `onErrorRetry` + the gated 「重试」 `TextButton` (`ErrorBlock.kt:28-33`, `:72-77`), `onImageClick` + the gated `clickable` (`UserMessageBlock.kt`, `ImageGridBlock.kt:47-51`, `:101-118`); `BlockRenderer.kt:21-56` now declares only the two live callbacks and its KDoc records that re-adding one means adding the surface it opens. Unused imports (`MaterialTheme` in `DiffBlock`, `TextButton` in `ErrorBlock`, `clickable` in `ImageGridBlock`) were removed with them. Still open as *new UI*, not wiring: a full-screen diff route, an image viewer, a real retry action | — |
...
| F22 | DEAD CODE | the `system-prompt` block kind cannot be produced | `rpc/Transcript.kt` → RPC | — | **rpc-side fix is impossible alone; labelled app-only instead** — pi has no `system_prompt` entry (`session-manager.ts:145-155`) and **no RPC command returns the prompt** (`rpc-types.ts:20-71`; `get_state`'s `RpcSessionState`, `:96-109`, has no prompt field), while pi never renders the prompt text at all (its `/context` listing prints only the source path, `interactive-mode.ts:1715-1722`, and the only reader of the text is the extension hook at `:2068`). `onSystemPrompt` has no caller in `app/**` (tests only). Both the item (`Transcript.kt:206`) and the hook (`:1945`) now say exactly that. Deleting is a **coordinated** change — item + `ui/blocks/SystemPromptBlock.kt` + `BlockRenderer.kt:71` + spec §7.4's row + the test — because deleting the item from `rpc/` alone makes `ui/**` fail to compile. | — |
| F23 | DEAD CODE | `outputTruncated` is never assigned, but is read for a label | `rpc/Transcript.kt` + `ui/blocks/ToolCallBlock.kt` → RPC + 行为 | — | **implemented** — `detailsTruncated` (`Transcript.kt:528`) reads `details.truncated` *and* `details.truncation`/`details.truncation.truncated`, and both finalize paths assign it (`:1233` update, `:1250` append), so `ToolCallBlock`'s 已截断 label is reachable. pi really does put it there: `core/tools/bash.ts:268` (`truncation: snapshot.truncation.truncated ? snapshot.truncation : undefined`), `core/tools/read.ts:156-165` (`details = { truncation }`), shape at `core/tools/truncate.ts:18-30`. Tests: `FidelityFixesTest:688` (truncated ⇒ true) and `:706` (`"truncation":null` ⇒ false). | — |
| F24 | DEAD CODE | 8 `message_update` delta kinds parse then fall into `None` | `rpc/Events.kt`, `rpc/Transcript.kt` → RPC | — | **done for every kind that carries content; the other four are correctly dropped** — the authoritative payloads overwrite the row: `text_end.content` (`Transcript.kt:1024`), `toolcall_end.toolCall` (`:1094`) and — closed this pass — `thinking_end.content` (`Events.kt:56` parses it, `Transcript.kt:1044` consumes it), tests `FidelityFixesTest:660`, `:674` and the three new ones. `thinking_end` is not merely corruption insurance: pi documents "Redacted thinking may be complete at start and emit no deltas" (`ai/src/types.ts:542-543`) and strips the cumulative `message` from the wire (`modes/json-event.ts:40-45`, `:56-60`), so it is the **only** source for such a block — before this the app dropped it entirely. The four still falling to `None` (`Transcript.kt:1133`) are right to: `text_start`/`thinking_start` keep only `contentIndex` and `start` only its type (their content lives in the cumulative `partial`, stripped by `json-event.ts:32-37`), `done.reason` is already covered by `message_end.stopReason` (`Events.kt:442`), and `toolcall_delta` would need pi's tolerant streaming-JSON parser (`ai/src/utils/json-parse.ts:104-124` + the `partial-json` dependency) for a row pi's own renderer does not build from the delta at all (`interactive-mode.ts:3244-3277` renders the accumulated message). | — |
| F25 | DEAD CODE | the `else -> NoticeBlock("暂不支持的内容块")` branch is unreachable | `ui/blocks/BlockRenderer.kt` → 行为 | — | **applied both halves (uncommitted)** — **app:** `ui/blocks/BlockRenderer.kt:83-88` — the `else` and its `NoticeBlock("暂不支持的内容块…")` are deleted, and the KDoc records why the branch could never run (`TranscriptItem` is sealed with exactly the 14 kinds matched above, so an unknown *item* cannot exist and a future subtype must fail the build instead of silently rendering as a notice). **rpc (point authorisation for this one branch):** `rpc/Transcript.kt:959-978` — `PiEvent.Unknown`'s non-entry path no longer returns `TranscriptChange.None`; it appends the same kind of deduplicated `Notice` the file already uses for an unknown assistant delta (`:1138-1144`), wording matched to it (`未知的事件：<type>` vs `未知的流式事件：<kind>`), keyed by kind so a repeated event cannot flood the transcript. Consistency was the reason: an unknown delta was visible and an unknown event was silent in the same reducer | — |
| F26 | DEAD CODE | dead `skill`/`skill_invocation` entry helpers and aliases (pi has no such entry type) | `rpc/Transcript.kt` → RPC | — | **implemented (a5ec766)** — deleted the `"skill"`/`"skill_invocation"` entry aliases and `onSkillEntry`, plus the three public helpers with no caller anywhere (`onSkill`, `onModelChange`, `onError`: `app/src/main` calls none of them, only tests did). `ENTRY_EVENT_TYPES` (`Transcript.kt:2065`) now holds only real pi entry types — the union is `session-manager.ts:145-155` — plus the one deliberate `custom_message` alias `hook_message`. Tests rewritten to pin that skill-/error-shaped records are inert; the real skill card still comes from `projectUser`'s `<skill …>` split (`_expandSkillCommand` rewrites the user message, `core/agent-session.ts`). | — |
...
| F30 | PERF | `formatClock` builds a `SimpleDateFormat` per call, per recomposition | `ui/blocks/BlockChrome.kt` → 行为 | RR-P8 | **implemented (uncommitted)** — `ui/blocks/BlockChrome.kt:187-205`: the per-call `SimpleDateFormat("HH:mm", Locale.getDefault())` is replaced by one hoisted `DateTimeFormatter` (`CLOCK_FORMAT`) plus `Instant.ofEpochMilli(ts).atZone(ZoneId.systemDefault()).format(...)`. Chosen over P8's literal `ThreadLocal<SimpleDateFormat>` for two reasons the review also allows ("or use `java.time.format.DateTimeFormatter`"): `DateTimeFormatter` is immutable/thread-safe, so there is no thread-local and nothing to leak, and the zone is still resolved per call — a cached `SimpleDateFormat` freezes the timezone it was built with, so after a device timezone change the per-call form was correct and the cached one would not be. Imports updated (`java.text.SimpleDateFormat`/`java.util.Date` out, `java.time.Instant`/`ZoneId`/`DateTimeFormatter` in; `Locale` still used by `formatChars`). Recorded limit: the pattern's locale is captured once per process instead of per call — no effect on `HH:mm` in this app's locales | — |
| F31 | PERF | `ToolCallBlock` re-scans/splits the whole output every composition | `ui/blocks/ToolCallBlock.kt` → 行为 | — | **implemented (uncommitted)** — `ui/blocks/ToolCallBlock.kt:46-92`: `lineCount(item.output)` is `remember(item.output)` (`outputLineCount`); the `headLines` split sits in `remember(item.output, expanded, fullOutput, outputOversize)` (`previewText`, also F17's 200-line / 展开全部 source); the footer — the `mutableListOf` + `joinToString` rebuilt per composition, whose `lineCount` scanned the whole output — is `remember(...)` over the row's scalar fields (`:84-92`), with `toolFooter` now taking the count (`:176-191`) instead of re-scanning. Net: no O(output) work and no line array per recomposition; the row only recomputes when its output actually changes (which F8's 200 ms publish throttle then bounds) | #55 (`outputMaxLines`) |
...
| F33 | PERF | `items(...)` has no `contentType`, so slots cannot be reused | `ui/screens/ChatScreen.kt` → 行为 | — | **implemented (uncommitted)** — `ui/screens/ChatScreen.kt:562-571`: the transcript's `itemsIndexed` now passes `contentType = { _, item -> item::class }` alongside its existing `key`; the sealed `TranscriptItem` subtypes are already the granularity a `LazyColumn` slot is reused at, so scrolling a long mixed session can now reuse the row structure across kind changes instead of re-inflating. (The review's F34 — the client-side slice + 加载更早 header — is the other half of this row's neighbourhood and was left to its own owner; the `contentType` was the part listed as patch-ready here) | — |
| F34 | PERF | nothing bounds the transcript or the entry list | `rpc/Transcript.kt` + `ui/screens/ChatScreen.kt` → RPC + 行为 | — | **rpc half deliberately has nothing to do — verifiable reason** — the review's own minimal fix is a render-side slice. The reducer's `items` must stay whole: (1) pi can only page **forward** (`docs/rpc.md:717-729`, `rpc-types.ts:65` — an entry id as a durable cursor), so a row the reducer dropped could never be fetched back; (2) the reducer's list is the client's only copy of the session and `seedFromHistory` rebuilds it wholesale on every attach; (3) spec §4.5's 加载更早 needs the older rows to exist client-side. The patch is therefore UI-only: slice the list for the `LazyColumn` with a 加载更早 header, plus F33's `contentType`. | — |
