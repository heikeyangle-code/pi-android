...
| **F32** / **RR-P9** | the five markdown config objects are rebuilt per composition | `ui/render/PiMarkdown.kt:76-93` | **NOT done — reverted.** The `remember` wrapper was illegal: the library's builders are `@Composable` and `remember`'s lambda is `@DisallowComposableCalls`, so the release build failed with "@Composable invocations can only happen from the context of a @Composable function" (CI run 1482257, step 9). Caching needs the builders to become pure functions of the palette, not a wrapper. |
...
| F32 | PERF | five markdown config objects rebuilt per composition | `ui/render/PiMarkdown.kt` → markdown | RR-P9 | **patch-ready again** (the `remember` wrapper did not compile; see §10.1) | — |
...
| RR F2 / F3 (pending tool cards; truncated/aborted answer reports nothing) | **Mitigated app-side; root fix is `rpc/**`** | `PiSessionViewModel.kt:675` projects pending `ToolCall`s as interrupted once the turn is not streaming, and `:1965` builds the row pi prints under a partial answer (`components/assistant-message.ts:182-200`) from `stopReason`. Both are live-only: the persisted session has no such entry, so a replay cannot reproduce them. The complete fix (reducer `MessageEnd`) is in the report to the parent, not applied. |
...
| RR F32/P9 (markdown config rebuilt per frame) | **Reverted — the fix did not compile** | `remember { piMarkdownColors() }` etc. were rejected by the Compose compiler (the builders are `@Composable`; `remember`'s lambda disallows composable calls). All five are now called directly. Needs a pure-function refactor of the builders in `ui/render/**`. |
...
F2/F3's reducer half; RR F6 (custom entries), F8 (200 ms tool throttle), F10–F34 — all of which live
in `rpc/**` or in files this agent does not own.
