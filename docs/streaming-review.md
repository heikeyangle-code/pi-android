...
| 命令 | 结果 |
|---|---|
| `bash tools/run-app-pure-checks.sh` | **我的两个 harness 全过**：`pure-checks: OK — tail-follow`（52 检查）、`pure-checks: OK — utf8-stream`（18 检查）。整脚本这次是 **FAILED — 2 of 12**，两条都在**别人的在制品**里：`pure-checks: FAILED — models-inventory did not compile`（`app/src/test/kotlin/app/pi/packages/PiModelInventoryCheck.kt:272`、`:286` 的 `cannot infer type for type parameter T`，是同一轮新增的检查文件）与 `pure-checks: FAILED — pre-spawn`（`FAIL the exposed flag is emitted: --offline`）。其余 8 个（`packages`/`guest-paths`/`shell-policy-mirror`/`mentions`/`agent-tool-paths`/`sessions`/`extension-error-text`/`settings-audit`）都是 OK。跑的是脚本的**快照副本**（`tools/.pure-checks-snapshot.sh`，跑完已删）—— 因为同一时刻别的代理正在往这个脚本里追加 harness，直接跑会被"读到写了一半的文件"打断（我第一次就撞上了：`line 312: :rpc: command not found`，那是别人写文件的中间态，不是脚本的错）。 |
