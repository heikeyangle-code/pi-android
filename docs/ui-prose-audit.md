...
| B4-5 | `:704` | `会话文件的存放目录。优先级是 --session-dir、PI_CODING_AGENT_SESSION_DIR，最后才是这项设置。改动需要重启 App。` | 1 | CLI flag + 环境变量 | `会话文件的存放目录。改动需要重启 App。` | 已改 → **该行已删除**（App 用启动参数把目录固定了，写设置也不会被读；见 `settings-review.md` §7.5） |
| B4-6 | `:742` | `App 侧对旧会话文件的清理策略。pi 本身不删除会话，删除动作始终可撤销。` | 2 | 「App 侧」「pi 本身不…」 | `旧会话文件的清理策略。删除动作始终可撤销。` | 已改 → **该行已删除**（没有消费者，pi 也没有任何 retention/prune 键；见 `settings-review.md` §2.2） |
...
> **更正（2026-09-20）**：该说法与代码不符 —— `DeviceUiAutomation.kt:328/439/473/1503` 无条件读 `node.text`，`app/src/main/kotlin/app/pi/bridge/` 全仓没有 `isPassword` / `inputType` / `TYPE_TEXT_VARIATION_PASSWORD` 任何过滤 ⇒ 无障碍读屏**并不排除**密码字段文本。界面上唯一与安全窗口相关的防护是截图拒绝（`DeviceUiAutomation.kt:1436`，`FLAG_SECURE`）。
