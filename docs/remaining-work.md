...
| §I2 动作类设置行 | 已处置：19 行逐行有结论 | **已实现（本轮复核一致）**：注册表现在 `kind = PiRowKind.Action,` **有 6 行**（TUI-only 裁决后又删了 `app.sessions.import`/`app.about.changelog`，见 `settings-review.md` §9.2）（`app.credentials.apiKey`/`.oauth`、`app.localModels.manage`、`app.compaction.runNow`、`app.sessions.import`、`app.security.emergencyStop`、**`app.runtime.restartEngine`（§I11 那一轮新增）**、`app.about.changelog`），其余已删 | `grep -c "kind = PiRowKind.Action," ui/settings/PiSettingsRegistry.kt` → **8**；`PiRoot` 的 `onRunAction` | 逐行判据（pi 有没有）在账本 §I2 表内，每条都有 `file:line` | 已交付 |
