...
3. **用户已决定不做的一类（不核 pi、不记债）**：玻璃/毛玻璃、装饰性动效、光晕等**纯视觉、空耗性能、没有信息增益**的东西。遇到这类条目直接标 `用户已决定不做`。本清单中这一类目前为空（F13/F14 是**对比度可读性**，不是装饰，见 §3）。
...
| 口径 | 已实现 | 仍未做 | 无法判断 / 未核实 |
|---|---|---|---|
| 本文件逐条核实的**条目**（§2–§5 的表行） | **31** | **10**（含"半边"） | **9** |
| 其中 purely `pi 有 <file:line>` 且我们没做的**真债** | — | **3**（§I11 UI 半边、§I7 的 4 个运行时行、F10 UI 半边） | — |
| 引账本自身已对账的分组（未逐行重核，见说明） | `feature-gaps` §7 的 30 条 · `gap-disposition` §9 的 29 条 · `fidelity-review` 10 条 | `extension-compatibility` 的 68 条 MISSING（多数为 TUI-only，未逐条重核） | `feature-gaps` 的 **112 行**（§7 自述未核实）· `§C` 真机 4 条 |

**一句话结论**：**能力债几乎见底了**。真正"pi 有、我们没有"的只剩 3 条（且都是 App 侧接线，不是新能力）；其余"未做"集中在 **rpc/engine 的性能与并发**（RR F9/F34）、**调色板与样式一致性**（F13/F14/F28/F29）、以及**新 UI**（F10/F16/F20）。账本里最危险的不是"没做"，是**还写着没做但其实做完了**——见 §6。
...
| §I2 动作类设置行 | 已处置：19 行逐行有结论 | **已实现（本轮复核一致）**：注册表现在 `kind = PiRowKind.Action,` **只有 7 行**（`app.credentials.apiKey`/`.oauth`、`app.localModels.manage`、`app.compaction.runNow`、`app.sessions.import`、`app.security.emergencyStop`、`app.about.changelog`）= 4 实现 + 3 跳转，其余 12 行已删 | `grep -c "kind = PiRowKind.Action," ui/settings/PiSettingsRegistry.kt` → **7**；`PiRoot.kt:167-236` 的 `onRunAction` | 逐行判据（pi 有没有）在账本 §I2 表内，每条都有 `file:line` | 已交付 |
| §I5 主题 JSON 到不了 App 颜色 | DEFECT：两套写死调色板 + 漏 `~/.pi/agent/themes` | **已实现**（两项都修了） | ①`MainActivity.kt:31-44` 收集 `session.theme` 并 `PiTheme(palette = theme.palette, dark = theme.dark)`（注释明写"不是两个手写常量"）；②`PiThemeFiles.scanDirectory(File(agentDir, "themes"))` (`:152`)、`(File(workspace, ".pi/themes"))` (`:153`)；③`PiThemeLoader.load` (`:202`)；④`PiPalette.kt` 只剩 `data class PiPalette`（无写死调色板） | `pi 有` `core/resource-loader.ts:815`/`:821`/`:875`（两个主题目录）、`modes/interactive/theme/theme.ts:232`（`resolveVarRefs`）、`:597`（`resolveThemeSetting`） | — |
...
| §I7 追加：4 个运行时只读行**没有写入方** | （本轮新发现，账本未记） | **仍未做（App 侧接线）**：`app.runtime.piVersion` / `nodeVersion` / `rootfsUsage` / `wakeLock` 只在注册表出现一次，全树无写入方，界面永远显示各自默认值（"未安装"/"未安装"/"未知"/"未知"），`G_RUNTIME` 摘要因此读成「pi 未安装…」 | `grep -rn "app.runtime.piVersion"` 只命中 `PiSettingsRegistry.kt:1494`（同类 `:1518`/`:1529`/`:1558`）；读侧 `:1757` | `pi 有但我们够不着`：pi 有版本上报（`cli/args.ts:93` `--version`、`:321`），但 **RPC 没有 version 命令**（`modes/rpc/rpc-types.ts:20-74`）；`rootfsUsage`/`wakeLock` 是 `pi 无对应物（App 自己的运行时/电源信息）` | `runtime/**` + `engine/**` |
...
| §I11 环境类能力（离线 / `--system-prompt` / `PI_CACHE_RETENTION`） | CLI-ONLY / MISSING；"环境变量映射是写死的" | **仍未做（只剩 UI 半边）**：引擎侧早已接好——`rpc/PiLaunchOptions.kt:31`/`:47`/`:61` 映射 `PI_OFFLINE`/`PI_CACHE_RETENTION=long`/`--system-prompt`/`--append-system-prompt`，`engine/PiEngineHost.kt:192`/`:215`/`:275`/`:316`/`:399` 应用并在 `restart()` 复用；**卡点**：没有任何 settings 行传值（`grep "离线\|systemPrompt\|cacheRetention" PiSettingsRegistry.kt` → 0），`boot()` 的唯一调用点 `PiSessionViewModel.kt:634` 只传 `workspaceProvider`（默认 `PiLaunchOptions()`） | 见左 | `pi 有` `cli/args.ts:110`（`--system-prompt`）、`:112`（append）、`:318`（`--offline`）、`:433`（`PI_OFFLINE` 环境变量）、`packages/ai/src/api/anthropic-messages.ts:57`（`PI_CACHE_RETENTION=long`）、`core/model-runtime.ts:196`（读 `PI_OFFLINE`） | **真债**：`ui/settings/**` + `PiSessionViewModel`（接线，不是新能力） |
...
| **F16** | 工具返回的图片被压成字面量 `[image]` | **rpc 半边已实现（`Events.kt:188` 保留 `List<PiImage>`）；UI 半边仍未做** | 缺失判据：没有把工具结果里的图片交给 `ImageGridBlock` 的渲染路径 | `pi 有` `packages/tui/src/components/markdown.ts:619-627`（终端渲染图片占位/alt） | 行为代理 |
...
| **F22** | `system-prompt` 块类型无法产生 | **已实现为有意偏离（App-only）**：pi **没有** system prompt 的转录组件（账本 §10.1 已核：只有 `interactive-mode.ts:1716-1719`/`:2068` 读它），所以这个块是 App 自己的表示 | `ui/blocks/SystemPromptBlock.kt` | `pi 无对应物（pi 没有该组件）` | markdown 代理（已记） |
...
| **F34** | 转录与条目列表没有任何上界 | **仍未做** | 缺失判据：列表无窗口化/上限；pi 侧 `get_entries` 是前向游标（不可回溯，见账本） | `pi 有但我们够不着`：pi 的 TUI 只渲染视口（`packages/tui`），RPC 的 `get_entries` 游标只能前进 | `rpc/**` + `ui/**` |
...
| §1.10 `PI_CACHE_RETENTION` | MISSING-GUI | **引擎已接、settings 未接**（= 本文件 §2 的 §I11，真债之一） |
...
### 5.2 `extension-compatibility.md`（68 条 MISSING）：**未核实（多数是 TUI-only，不是债）**

只核了两类：

| 条目 | 核实结果 | pi 有对应物吗 |
|---|---|---|
| `sendMessage` / `appendEntry` / 自定义 message/entry 的**渲染** | **部分已实现**：`MessageEnd.customType`/`display` 已解析并走 `onHookMessage`（`rpc/Events.kt:140-146`、`Transcript.kt:663`），`onCustomEntry` 处理 `"custom"`（= RR F6/RR-P4 `applied`） | `pi 有` `core/extensions/types.ts:1365-1381` |
| `registerMessageRenderer` / `registerEntryRenderer` / `registerMarkdownTransformer` / `ctx.ui.custom()` 等 | **pi 有但我们够不着**：这些 hook 只在 pi 自己的 TUI 里被调用，`--mode rpc` 下 `custom()` 是 no-op（账本 F3/D3 已定） | `pi 有但我们够不着（TUI-only，RPC 无通道）` |
| 其余 ~60 条 | **未核实**（每条要回读 `ExtensionAPI` 与 `rpc/**`；本轮时间为真机/账本核实让路） | 大多预计是上两类之一，但不写没有核过的结论 |
...
| `extension-compatibility.md` 约 60 条 MISSING | 未逐条回读 | 每条 1 次 `grep` + 1 次 pi 侧 `grep` |
...
1. **§I11 的三个环境开关接上 UI**（`PiLaunchOptions` 已就绪，缺 settings 行 + `boot()` 传值）——**唯一一条"pi 有、引擎已接、只差 UI"的真债**，落地面小、用户可感知（离线模式是手机流量的实际开关）。pi 依据：`cli/args.ts:110`/`:318`/`:433`、`anthropic-messages.ts:57`。归属：`ui/settings/**` + `PiSessionViewModel`。
2. **§I7 的 4 个运行时只读行接上供给方**（`piVersion`/`nodeVersion`/`rootfsUsage`/`wakeLock` 现在永远显示"未安装/未知"）——**界面在展示假信息**，比缺功能更坏；`pi --version` 有对应物（`cli/args.ts:93`），其余两项是 App 自己的信息。归属：`runtime/**` + `engine/**`。
...
5. **RR F16 的 UI 半边**（工具返回的图片显示为字面量 `[image]`）——rpc 已经把 `List<PiImage>` 保下来了（`Events.kt:188`），只差渲染接线；现在用户看到的是**明显错误的输出**。

> 备注：**纯视觉空耗类（玻璃/动效/光晕）本轮没有条目落在这一类**；F13/F14 是对比度可读性、F28/F29 是手势与间距一致性，都不是装饰性效果，故未标 `用户已决定不做`。
...
| `feature-gaps.md` §1.10 | `PI_CACHE_RETENTION` MISSING-GUI | 引擎已接，只剩 settings（见 §I11） |
