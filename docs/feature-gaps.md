...
| **Action-kind settings rows** | — | **wired.** `PiRoot` passes `onRunAction`, so no Action row can fall through to "还没有接入实现" any more. **6** rows remain (`app.credentials.apiKey` / `.oauth`, `app.localModels.manage`, `app.compaction.runNow`, `app.security.emergencyStop`, `app.runtime.restartEngine`); `app.sessions.import` and `app.about.changelog` were deleted by the TUI-only ruling (`settings-review.md` §9.2); the credential pair opens its own screen via `PiSettingsStack.hostActions`, and the other 11 rows were deleted in the §I2 pass. `SettingsGroupScreen` now says "这个入口当前不可用。" and nothing reaches it | **IMPLEMENTED** | Yes | — (see §2.9) |
