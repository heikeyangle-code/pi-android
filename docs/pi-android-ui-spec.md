...
**pi 主题 JSON 是颜色的唯一事实源**（53 个必需令牌 + 可选 `vars` 与 `export` 段）。App 把它映射到 M3 角色，于是**换 pi 主题 = 换 App 主题**，且用户在桌面自制的主题能直接导入。
...
- 支持**导入 pi 主题 JSON**（从文件、从剪贴板、从 URL）。校验 53 个必需令牌，缺项用当前主题补并给出警告列表。
